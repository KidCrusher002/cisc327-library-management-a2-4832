...
def test_borrow_success(monkeypatch):
    ...
    monkeypatch.setattr("library_service.get_book_by_id", lambda id: fake_book)
    monkeypatch.setattr("library_service.get_patron_borrow_count", lambda p: 2)
    monkeypatch.setattr("library_service.insert_borrow_record", lambda p, b, d, due: True)
...
def test_borrow_book_not_found(monkeypatch):
    monkeypatch.setattr("library_service.get_book_by_id", lambda id: None)
    success, msg = borrow_book_by_patron("123456", 1)
    assert not success
    assert "Book not found" in msg

def test_borrow_no_copies(monkeypatch):
    monkeypatch.setattr("library_service.get_book_by_id", lambda id: {"available_copies": 0})
    success, msg = borrow_book_by_patron("123456", 1)
    assert not success
...
def test_borrow_limit_bug(monkeypatch):
    ...
    monkeypatch.setattr("library_service.get_book_by_id", lambda id: fake_book)
    monkeypatch.setattr("library_service.get_patron_borrow_count", lambda p: 5)  # Should fail, but bug lets it pass
    monkeypatch.setattr("library_service.insert_borrow_record", lambda p, b, d, due: True)
    monkeypatch.setattr("library_service.update_book_availability", lambda b, c: True)
